...
class CsvPropertiesDialog(Dialog):
    def __init__(self, callback=None, abort_callback=None) -> None:
        super().__init__(
                            tag=f"{self.tag_prefix}.window.csv_properties_dialog",
                            width=300,
                            height=220,
                            no_close=False,
                            callback=callback,
        ...
        set_csv_callback = lambda x:(
            self.callback(None, None, {"delimiter":self.delimiters[dpg.get_value(f"{self.tag}.delimiter")], 
                                       ...
                                       "dialect":dpg.get_value(f"{self.tag}.dialect")}),) # type: ignore
        ...
        dpg.add_button(label="Cancel", pos=(self.width-60,self.height-30), callback=self.abort_callback, parent=self.tag) # type: ignore
